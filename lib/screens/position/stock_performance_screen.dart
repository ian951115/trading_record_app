...
class StockPerformanceScreen extends StatefulWidget {
  ...
  @override
  State<StockPerformanceScreen> createState()
      => _StockPerformanceScreenState();
}

enum _PerfFilter { all, open, closed, profit, loss }

class _StockPerformanceScreenState
    extends State<StockPerformanceScreen> {
  _PerfFilter _filter = _PerfFilter.all;

  List<StockPerformance> _applyFilter(
      List<StockPerformance> all) {
  ...
  Widget build(BuildContext context) {
    ...
    final totalRealized = allPerfs.fold(
      0.0, (s, p) => s + p.totalRealizedPnL);
    final winPerfs = allPerfs
      .where((p) => p.totalSellCount > 0);
    final overallWinRate = winPerfs.isEmpty ? 0.0
      : winPerfs.map((p) => p.winRate)
          .reduce((a,b) => a + b) / winPerfs.length;
      ...
      body: Column(children: [
          ...
          child: StatsStrip(cells: [
            StatCell(label: '總買入',
              value: formatter.format(
                allPerfs.fold(0.0,
                  (s,p)=>s+p.totalBuyAmount).toInt())),
            StatCell(label: '總賣出',
              value: formatter.format(
                allPerfs.fold(0.0,
                  (s,p)=>s+p.totalSellAmount).toInt())),
            StatCell(
              label: '平均勝率',
              value: '${(overallWinRate*100).toStringAsFixed(0)}%',
            ),
        ...
        Expanded(
          child: filtered.isEmpty
            ? const Center(child: Text('沒有符合的紀錄',
            ...
            Expanded(child: Column(
              ...
              children: [
                Row(children: [
                  ...
                  if (p.isOpen) ...[ const SizedBox(width:6),
                    Container(
                      padding: const EdgeInsets.symmetric(
          ...
          AnimatedCrossFade(
            ...
            secondChild: Padding(
              ...
              child: Column(children: [
                ...
                const SizedBox(height: 10),
                Row(children: [
                  Expanded(child: _DItem(label:'買入成本',
                    value:f.format(p.totalBuyAmount.toInt()))),
                  Expanded(child: _DItem(label:'賣出收入',
                    value:f.format(p.totalSellAmount.toInt()))),
                  Expanded(child: _DItem(label:'勝率',
