...
// 二代健保計算
double _calcHealthInsurance(double amount) {
...
class AddDividendScreen extends StatefulWidget {
  ...
  @override
  State<AddDividendScreen> createState()
      => _AddDividendScreenState();
}

class _AddDividendScreenState
    extends State<AddDividendScreen> {
  ...
  // 計算毛額
  double get _grossAmount => _pricePerShare * _heldShares;
  // 自動計算二代健保
  double get _healthInsurance =>
    _calcHealthInsurance(_grossAmount);
  // 淨額
  double get _netAmount =>
    _grossAmount - _fee - _healthInsurance;
  ...
  void _save() async {
    ...
    final div = Dividend(
      ...
      name: _name,
      type: _type,
      cashAmount: _type == DividendType.cash
          ? _grossAmount : 0,
      shareAmount: _type == DividendType.stock
          ? _shareAmount : 0,
      fee: _fee,
      healthInsurance: _type == DividendType.cash
          ? _healthInsurance : 0,
      note: _note.isEmpty ? null : _note,
    );

    await context.read<DividendRepository>()
        .addDividend(div);
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            FormCard(child: Row(children: [
              Expanded(child: GestureDetector(
                onTap: () => setState(
                  () => _type = DividendType.cash),
            ...
            FormCard(child: Column(children: [
              ...
              _AppField(
                ...
                onChanged: (v) => setState(() {
                  _symbol = v;
                  // 同 add_trade_screen 的 mockStockMap 查詢
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Text(
            ...
            style: const TextStyle(
              fontSize:13, color:Color(0xFF1A1F2E))),
        ]),
      ),
    );
  }
...
// 引用 add_trade_screen 已有的 mockStockMap
// 建議之後把 mockStockMap 提取到 lib/utils/stock_data.dart
